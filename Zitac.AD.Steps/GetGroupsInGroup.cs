...
namespace Zitac.AD.Steps
{
    ...
    [Writable]
    public class GetGroupsInGroup : BaseFlowAwareStep, ISyncStep, IDataConsumer, IDataProducer, IDefaultInputMappingStep //, INotifyPropertyChanged
    {
        ...
        public ResultData Run(StepStartData data)
        {
            ...
            try
            {
                ...
                List<SearchResultEntry> Group = LDAPHelper.GetPagedLDAPResults(connection, BaseDN, SearchScope.Subtree, Filter, new List<string> { "objectSid", "distinguishedname", "primaryGroupId" }).ToList();
                ...
                if (RecursiveSearch)
                {
                    Filter = "(&(objectClass=group)(objectCategory=group)(memberOf:1.2.840.113556.1.4.1941:=" + FoundGroup.Attributes["distinguishedname"][0] + "))";
                }
                else
                {
                    Filter = "(&(objectClass=group)(objectCategory=group)(memberOf=" + FoundGroup.Attributes["distinguishedname"][0] + "))";
                }
